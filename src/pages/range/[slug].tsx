import React from "react";
import type { GetStaticPaths } from "next";
import AltProductRanges from "@components/pages/range/altProductRanges";
import "react-medium-image-zoom/dist/styles.css";
import { useDispatch } from "react-redux";
import { useRouter } from "next/router";
import HeroProducts from "@components/pages/range/HeroProducts";
import ProductLayout from "@layouts/ProductLayout";
import AddressProduct from "@components/pages/range/AddressProduct";
import ProductDescription from "@components/pages/range/ProductDescription";
import ProductSpecification from "@components/pages/range/ProductSpecification";
import { useTheme } from "@emotion/react";

interface IFormInputs {
  name: string;
  code: string;
  quantity: number;
  coverage: number;
  showAge: boolean;
  age: number;
}

export default function Page(props: any) {
  const idRouter = useRouter();
  const data = props.motif.data[0].attributes.motif.data.attributes;
  // props.product.data.attributes.motif.data.attributes.products.data;
  // console.log(props.testing.data.attributes.motif.data.attributes);
  // const motifNames = props.testing.data.map(item => item.attributes.motif.data.attributes.Name);
  // console.log(data);
  // console.log("iaojsdjajsdioasidjjasoidjasoijioasjiojasd");
  // console.log(props.product.data[0].attributes.Name);

  return (
    <>
      <HeroProducts
        imageHero={props.motif.data[0].attributes.motif.data.attributes.Image_Hero_2880x1138px.data?.attributes.url}
        nameMotif={data.Name}
        NColor={data.N_Color}
        NDimension={data.N_Dimension}
        NFinish={data.N_Finish}
        ShortDescription={props.motif.data[0].attributes.motif.data.attributes.Short_Description}
      />
      <ProductLayout>
        <AddressProduct props={props} address={idRouter} />
        <ProductDescription props={props} data={data} hightlight={idRouter} />
      </ProductLayout>
      <ProductLayout backgroundColor={"#f5f5f5"}>
        <ProductSpecification props={props} data={data} />
      </ProductLayout>
      <ProductLayout>
        <AltProductRanges alt1={props.alternative1} alt2={props.alternative2} alt3={props.alternative3} />
      </ProductLayout>
    </>
  );
}

export const getStaticPaths: GetStaticPaths = async () => {
  const headers = {
    Authorization:
      "Bearer ",
  };

  const res = await fetch("https://strapi-app-tnshv.ondigitalocean.app/api/products?pagination[page]=0&pagination[pageSize]=999", {
    headers: {
      Authorization: `Bearer `,
    },
  });

  const response = await res.json();

  let temPath: any = [];
  response.data.map((item: any, index: any) => {
    temPath.push({
      params: {
        id: item.id.toString(),
        slug: item.attributes.Slug,
      },
    });
  });

  return {
    paths: temPath,
    fallback: false,
  };
};

export const getStaticProps = async ({ params }: any) => {
  console.log(params.slug);

  const responseMotif = await fetch("https://strapi-app-tnshv.ondigitalocean.app/api/products?filters[Slug][$eq]=" + params.slug + "&populate=deep,10", {
    headers: {
      Authorization: `Bearer `,
    },
  });
  const motif = await responseMotif.json();

  const responseProduct = await fetch("https://strapi-app-tnshv.ondigitalocean.app/api/products?populate[motif][populate][products][populate]=*&filters[Slug][$eq]=" + params.slug   + "&populate=deep,10", {
    headers: {
      Authorization: `Bearer `,
    },
  });
  const product = await responseProduct.json();

  // const responseMotif = await fetch("https://strapi-app-tnshv.ondigitalocean.app/api/products/" + params.id + "?populate[motif][populate]=*", {
  //   headers: {
  //     Authorization: `Bearer `,
  //   },
  // });

  // const responseAmbience = await fetch("https://strapi-app-tnshv.ondigitalocean.app/api/products/" + params.id + "?populate=*", {
  //   headers: {
  //     Authorization: `Bearer `,
  //   },
  // });

  // const product = await responseProduct.json();
  // const motif = await responseMotif.json();
  // const ambience = await responseAmbience.json();

  const responseAlt1 = await fetch("https://strapi-app-tnshv.ondigitalocean.app/api/motifs/" + 1 + "?populate=*", {
    headers: {
      Authorization: `Bearer `,
    },
  });

  const responseAlternative1 = await responseAlt1.json();

  const responseAlt2 = await fetch("https://strapi-app-tnshv.ondigitalocean.app/api/motifs/" + 2 + "?populate=*", {
    headers: {
      Authorization: `Bearer `,
    },
  });

  const responseAlternative2 = await responseAlt2.json();

  const responseAlt3 = await fetch("https://strapi-app-tnshv.ondigitalocean.app/api/motifs/" + 3 + "?populate=*", {
    headers: {
      Authorization: `Bearer `,
    },
  });

  const responseAlternative3 = await responseAlt3.json();

  return {
    props: {
      product: product,
      motif: motif,
      // testing: testing,
      // productOnly: ambience,
      alternative1: responseAlternative1,
      alternative2: responseAlternative2,
      alternative3: responseAlternative3,
    },
  };
};
