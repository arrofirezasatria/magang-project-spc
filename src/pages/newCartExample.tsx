...
import {
  Box, Typography, Button, Grid, Link, TableContainer, Table, TableHead, TableRow, TableBody, TableCell, FormControl, useMediaQuery, useTheme, TextField, IconButton, Divider,
} from "@mui/material";
import swr from 'swr'
  ...
  return (
    <>
      ...
      <ProductLayout backgroundColor="#f2f1f0">
        ...
        <Box className='cart' sx={{ width: '100%', my: { xs: '0', md: '20px' } }}>
          <Typography component='h1' sx={{ fontWeight: 'medium' }}>My cart {'(2)'}</Typography>
          <TableContainer sx={{
            mt: { xs: '0', sm: '20px' },
          }}>
            <Table
              ...
              {isMobile ? (
                <>
                  <Box className='cart-mobile'>
                    {data?.data?.attributes?.motif.data.attributes?.products.data.map((item, index) => (
                      <Box className='cart-row' sx={{
                        my: '1rem',
                        py: '1rem',
                      }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                          <Box sx={{ width: '100%', display: 'flex' }}>
                            <Box sx={{
                              ...
                              <Image src={item.attributes.Image_Tile_Face.data[0].attributes.formats.thumbnail.url} alt='' fill />
                            </Box>
                            <Box sx={{ pl: '1rem', display: 'flex', flexDirection: 'column', gap: '2px' }}>
                              <Typography sx={{ fontSize: '16px', lineHeight: '1.5em', fontWeight: '500' }}>
                                {item.attributes.Name}
                              </Typography>
                              <Box display="flex">
                                <Typography sx={{
                                  fontSize: '12px', lineHeight: '1.5em', fontWeight: '400'
                                }}>
                                  {item.attributes.Code}
                                </Typography>
                                <Divider orientation="vertical" flexItem sx={{ mx: '10px' }} />
                                <Typography sx={{ fontSize: '12px', lineHeight: '1.5em' }}>
                                  {item.attributes.tile_dimension.data.attributes.Dimension}
                                </Typography>
                              </Box>
                              <Box>
                                <Typography sx={{ fontSize: '14px', lineHeight: '1.25em' }}>
                                  <NumericFormat
                ...
                <>
                  ...
                  <TableBody>
                    {data?.data?.attributes?.motif.data.attributes?.products.data.map((row, index) => (
                      <TableRow key={index} sx={{
                        ...
                        <TableCell align="left">
                          <Box sx={{
                            ...
                            <Image src={row.attributes.Image_Tile_Face.data[0].attributes.formats.thumbnail.url} alt='' fill />
                          </Box>
                        </TableCell>
                        <TableCell align="left">{row.attributes.Name}</TableCell>
                        <TableCell align="left">{row.attributes.tile_dimension.data.attributes.Dimension}</TableCell>
                        <TableCell align="left">{row.attributes.Code}</TableCell>
                        <TableCell align="left">
          ...
          <Box sx={{ display: { xs: 'block', sm: 'flex' }, justifyContent: 'space-between', mt: '40px' }}>
            ...
            <Box sx={{ p: '5px 15px', border: '2px solid #000', borderRadius: '5px', bgcolor: '#000', textAlign: 'center' }}>
