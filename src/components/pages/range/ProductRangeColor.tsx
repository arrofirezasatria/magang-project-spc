...
export default function ProductRangeColor({ props, pageTitle }: any) {
  ...
  .sort((a: any, b: any) => {
    if (activeTab === "new") {
      return b.attributes.motif.data?.attributes.isNew - a.attributes.motif.data?.attributes.isNew;
    }
    return 0;
  })
        ...
        <Grid display="flex" flexDirection="column" sx={{ position: "relative" }}>
          <Grid container spacing={2}>
            {filteredAndSortedData.map((item: any, index: React.Key | null | undefined) => {
              console.log("ini adalah image_tile_face");
              const ext = item.attributes.Image_Tile_Face?.data?.[0]?.attributes?.formats?.large?.url;
              if (ext) {
                console.log(ext);
              } else {
                console.log("ext is not available");
              }
              return (
                <Grid item key={index} xs={6} md={3} lg={3}>
                  <Link href={`/range/${item ? item.Slug : 153}`} key={index} style={{ color: "black", textDecoration: "none" }}>
                    <Box sx={{ cursor: "pointer" }}>
                      ...
                      >
                        {item.attributes.motif.data?.attributes.isNew && (
                          <Box
                            sx={{
                              backgroundColor: "black",
                              width: "65px",
                        ...
                        {/* <Image layout="fill" alt="ads" src={item.attributes.motif.data?.attributes.Image_Thumbnail_350px.data.attributes.url} /> */}
                        <Image layout="fill" alt="ads" src={item.attributes.Image_Ambience?.data?.[0]?.attributes?.formats?.large?.url} style={{objectFit:"cover"}} />
                      ...
                      >
                        ...
                        >
                          {item.attributes.motif.data?.attributes.product_varians.data.length > 0 ? (
                            item.attributes.motif.data.attributes.product_varians.data.map((varian: any, index: number) => {
                          ...
                          {item.attributes.motif.data?.attributes.style_motifs.data.length > 0 ? (
                            <>
                              <Box
                                sx={{
                                  display: "flex",
                              ...
                              >
                                ...
                                >
                                  {item.attributes.motif.data.attributes.style_motifs.data[0].attributes.Style}
                                </Typography>
                              </Box>
                              {item.attributes.motif.data.attributes.style_motifs.data.length > 1 && (
                                <Box
                                  sx={{
                                    display: "flex",
                                    flexDirection: "row",
                        ...
                        >
                          <Tooltip title="Colours" arrow>
                            <Box display="flex" flexDirection="row">
                              ...
                              >
                                0{item.attributes.motif.data?.attributes.N_Color}
                              </Typography>
                            </Box>
                          </Tooltip>
                          <Tooltip title="Sizes" arrow>
                            <Box display="flex" flexDirection="row">
                              ...
                              >
                                0{item.attributes.motif.data?.attributes.N_Dimension}
                              </Typography>
                            </Box>
                          </Tooltip>
                          <Tooltip title="Finishes" arrow>
                            <Box display="flex" flexDirection="row">
                              ...
                              >
                                0{item.attributes.motif.data?.attributes.N_Finish}
                              </Typography>
                            </Box>
                          </Tooltip>
                        </Box>
