import { Box, Button, Grid, Typography, Divider, MenuItem, Toolbar, Link, Popover, Collapse, } from "@mui/material";
...
export default function TheProduct(props: any) {
  ...
  const showTheProduct = props.showp.data.attributes?.motif.data.attributes?.products;

  return (
    <>
      <Box>
        <Typography sx={{ fontSize: '16px', fontWeight: 'medium', letterSpacing: '2px', mb: '10px' }}>RELATED PRODUCTS:</Typography>
            ...
            <Grid item key={index} xs={12} md={6} sx={{ mt: '10px' }}>
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                <Box sx={{
                  ...
                  <Image src={item.attributes?.Image_Tile_Face.data[0].attributes?.formats.thumbnail.url} alt="" fill style={{
                    objectFit: 'cover'
                  }} />
                </Box>
                <Box sx={{ ml: '10px' }}>
                  ...
                  <Typography sx={{ fontSize: '12px', fontWeight: '400', color: '#111' }}>
